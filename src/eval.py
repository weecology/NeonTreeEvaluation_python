...
from src.field_crowns import field_crowns
from src.image_crowns import image_crowns

def evaluate_image_crowns(path=None, df = None, shp=None, project=False, show=True, iou_threshold=0.4):
...
def evaluate_field_crowns(path=None, df = None, shp=None, project=False, show=True, iou_threshold=0.4):
    """Field annotated crown evaluation routine
    submission can be submitted as a .shp, existing pandas dataframe or .csv path
    ...
    results = [ ]
    for name, group in df.groupby("plot_name"):
        result = field_crowns(df=group, project=project, show=show)
        results.append(result)
